...
Agent coordination follows `specs/AGENT_OPERATING_PROTOCOL.md`.
...
- Store the diagrams in a versioned project document under `specs/`.
...
- Prepare lint command structure so changed-file lint checks can be enforced by Git hooks later.
...
- Align role screens with the Signal UI design system direction documented in `specs/SIGNAL_UI_GUIDE.md`.
...
- Use extracted Signal UI tokens and component decisions from `specs/SIGNAL_UI_GUIDE.md` only as low-fi guidance; final hi-fi styling remains a later polish step.
...
- Apply Signal-derived colors, typography guidance, radius, spacing, button, text-field, option-card, badge/label, and bottom-sheet decisions from `specs/SIGNAL_UI_GUIDE.md`.
...
- Maintain `specs/TASK_PRESENTATION_BRIEF.md` as a stakeholder-readable summary of all implementation tasks.
...
- Maintain `specs/E2E_TEST_CASES.md` with detailed end-to-end cases in standard format.
...
- `specs/E2E_TEST_CASES.md` exists and covers Station, Gate, Terminal, Scout, security, and generic activity tariff flows.
