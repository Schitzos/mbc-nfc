...
All agents must treat the `specs/` folder as the project source of truth.
...
- `specs/TASKS.md` is the full task inventory.
- `specs/EXECUTION_ORDER.md` is the practical feature-based execution sequence.
...
The receiving owner must update or request updates to the relevant `specs/` document before downstream implementation continues.
...
1. Read the relevant `specs/` files for its role.
2. Check `specs/EXECUTION_ORDER.md` to confirm the currently active task.
3. Open the matching task section in `specs/TASKS.md`.
...
- Agents work only on the active task from `specs/EXECUTION_ORDER.md`, unless explicitly asked to review, clarify, or prepare a dependency for that task.
...
- Real NFC findings must be recorded in `specs/DEVICE_TEST_MATRIX.md`.
...
- If a behavior is not stated in `specs/`, agents must not invent it.
