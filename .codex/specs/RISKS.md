...
| R-003 | Sensitive identity and balance can be read by generic NFC app                                       | Medium     | High   | Implement Silent Shield with authenticated encryption, HMAC validation, opaque NFC envelope, and external NFC reader verification                         | Open   |
| R-004 | Encryption/signing secret is exposed through source control, logs, or app bundle                    | Medium     | High   | Never commit real secrets; use secure config, platform secure storage where available, redacted logs, and document fleet key rotation as future hardening | Open   |
