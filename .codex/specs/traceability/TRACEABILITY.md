...
| SR-001 to SR-013         | Covered by functional requirements FR-001 to FR-015, NFC availability handling, and their task/test mappings below. |
...
| FR-002 Station Registration                   | `DESIGN.md` Sections 5, 7, 8                                                                        | T-012, T-017, T-018, T-019, T-022, T-026A, T-026B, T-026C        | Use-case test; NFC device registration test                                                                    |
| FR-003 Station Top-Up                         | `DESIGN.md` Sections 5, 8                                                                           | T-013, T-017, T-022, T-026A, T-026B, T-026C                      | Use-case test; NFC top-up test                                                                                 |
| FR-003 Balance Cap (MAX_CARD_BALANCE)         | `DESIGN.md` Section 5 (domain config, error codes)                                                  | T-BUGFIX-002                                                     | top-up-balance-cap.use-case.test.ts                                                                            |
...
| FR-005 Gate Simulation Mode                   | `REQUIREMENTS.md` FR-005; `DESIGN.md` Section 10; `CARD_DATA_SECURITY_LEDGER_SPEC.md` Section 2.3   | T-FEAT-GATE-001                                                  | Simulation toggle, DateTimePicker, card `s:1` flag, Terminal skip-deduction; NFC-005A/NFC-005B device tests    |
...
| FR-016 NTAG215 Target Tag and Compact Payload | `REQUIREMENTS.md` FR-016; `CARD_DATA_SECURITY_LEDGER_SPEC.md` Section 2/2A; `DEVICE_TEST_MATRIX.md` | T-019, T-020, T-028                                              | NTAG215 capacity, compact payload, latest 5 logs fit, capacity failure path                                    |
| FR-017 NFC Operational Log Panel              | `REQUIREMENTS.md` FR-017; `DESIGN.md` Section 10                                                    | T-026E                                                           | Role-screen log toggle, clear action, and safe operational line validation                                     |
| FR-015 Operational Edge Case Handling         | `REQUIREMENTS.md` FR-015; `DESIGN.md` Section 8; `EDGE_CASES.md`                                    | T-014, T-018, T-019, T-020, T-023, T-024, T-027, T-028           | Invalid duration, write failure, capacity, and recovery tests                                                  |
| XFR-001 NFC Capacity and Write Verification   | `CARD_DATA_SECURITY_LEDGER_SPEC.md` Sections 2A, 10; `DESIGN.md` Section 8                          | T-018, T-019, T-020, T-028                                       | Capacity guard, write failure handling, device matrix evidence                                                 |
...
| NFR-006 Android-first NFC validation          | `DESIGN.md`, `DEVICE_TEST_MATRIX.md`                                                                | T-003, T-004, T-028                                              | Android + NTAG215 read/write testing; iOS deferred/best-effort unless validated                                |
...
| NFR-023 Troubleshooting observability         | `REQUIREMENTS.md` FR-017; `DESIGN.md` Section 10                                                    | T-026E                                                           | NFC operational log panel is toggleable, clearable, and redacted from sensitive data                           |

## Coverage Rules

- Every Must requirement must map to at least one design section, one task, and one test.
- Every card write requirement must include capacity validation and a failure path. writeNdefMessage throws on write failure.
...
| NFR-022 Firebase App Distribution Release Automation | `REQUIREMENTS.md`; `RELEASE_PLAN.md`                            | T-027B, T-030        | `.github/workflows/build.yml` proof: PR validation + `main` distribution evidence |
