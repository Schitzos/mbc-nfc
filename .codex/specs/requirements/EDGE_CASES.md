...
| EC-006 | Gate simulation uses future time           | Reject — simulation DateTimePicker only allows past times. If selected time is in the future relative to device clock, check-in is blocked with `INVALID_TIME`.                                                              |
...
| EC-016 | Tampered payload                           | All roles reject with `CARD_TAMPERED` error. Station shows the same confirmation prompt as for already-registered cards (Wipe & Re-register / Skip) only for unrecognized payloads, not for tampered authenticated payloads. |
...
| EC-020 | Top-up amount input                        | Top-up accepts numeric input (free-text allowed) with validation to ensure only numbers are entered. Preset buttons (10k/20k/50k/100k) are also available as shortcuts. Non-numeric input is rejected.                       |
